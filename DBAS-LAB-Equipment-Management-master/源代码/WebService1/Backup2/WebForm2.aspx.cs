	...
	public class WebForm2 : System.Web.UI.Page
	{
		...
		private void Button1_Click(object sender, System.EventArgs e)
		{
			...
			for(i=0;i<db1.getdata().Tables["basicinfo"].Rows.Count;i++)
			{
				...
				int r=ts.TotalDays.ToString().IndexOf("-",0,ts.TotalDays.ToString().Length);
				string f;
				if(i!=-1)
					f=((int)ts.TotalDays).ToString();
				else
				{
					f="-";
					int p=ts.TotalDays.ToString().IndexOf(".",0,ts.TotalDays.ToString().Length);
					f+=ts.TotalDays.ToString().Substring(i+1,p-i-1);
				}
				lbl.Text = f;
