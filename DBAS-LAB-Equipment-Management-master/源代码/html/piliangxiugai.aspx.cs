...
namespace html
{
	...
	/// piliangxiugai 的摘要说明。
	/// </summary>
	public class piliangxiugai : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				...
				{
					this.DropDownList5.Items.Add(new ListItem(db1.getdata().Tables["changjia"].Rows[i]["厂家名称"].ToString()));
				}
				sql1="select * from basicinfo where 设备名称id="+sb+" and 实验室id="+lab+" and 设备编号='"+this.DropDownList1.SelectedValue.ToString()+"'";
				db1.cleardata();
				db1.ExectueSQL("basicinfo",sql1);

				this.TextBox1.Text=db1.getdata().Tables["basicinfo"].Rows[0]["出厂日期"].ToString();
				...
				this.DropDownList5.SelectedValue=db1.getdata().Tables["changjia"].Rows[0]["厂家名称"].ToString();
			}
		}
		...
		{
			string sql1="update basicinfo set 厂家id=";

			string sql2="select * from changjia where 厂家名称='"+this.DropDownList5.SelectedValue.ToString()+"'";
			db1.cleardata();
			db1.ExectueSQL("changjia",sql2);

			string changjia=db1.getdata().Tables["changjia"].Rows[0]["厂家id"].ToString();
			sql1+=changjia;
			sql1+=",出厂日期='"+this.TextBox1.Text.ToString()+"' where 设备名称id=";

			sql2="select * from sbname where 设备名称='"+this.DropDownList4.SelectedValue.ToString()+"'";
			db1.cleardata();
