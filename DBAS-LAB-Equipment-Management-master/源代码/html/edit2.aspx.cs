...
namespace html
{
	...
	public class edit2 : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				this.Button1.Attributes.Add("onclick","return confirm('确定修改吗？')");
				string sql3="select * from basicinfo where 设备id=" + Request.QueryString["id"];
				db1.cleardata();
				db1.ExectueSQL("basicinfo",sql3);

				this.TextBox1.Text=db1.getdata().Tables["basicinfo"].Rows[0]["设备编号"].ToString();
				...
				sql1="select * from sbname where 设备名称id="+db1.getdata().Tables["basicinfo"].Rows[0]["设备名称id"].ToString();
				db2.ExectueSQL("sbname",sql1);

				this.DropDownList1.SelectedValue=db2.getdata().Tables["sbname"].Rows[0]["设备名称"].ToString();
				...
				sql1="select * from labname where 实验室id="+db1.getdata().Tables["basicinfo"].Rows[0]["实验室id"].ToString();
				db2.ExectueSQL("labname",sql1);
				this.DropDownList2.SelectedValue=db2.getdata().Tables["labname"].Rows[0]["实验室名"].ToString();
				...
				sql1="select * from changjia where 厂家id="+db1.getdata().Tables["basicinfo"].Rows[0]["厂家id"].ToString();
				db2.ExectueSQL("changjia",sql1);
				this.DropDownList3.SelectedValue=db2.getdata().Tables["changjia"].Rows[0]["厂家名称"].ToString();

				this.TextBox2.Text=db1.getdata().Tables["basicinfo"].Rows[0]["出厂日期"].ToString();
			}
		}
		...
		{
			string sql1="update basicinfo set 设备编号='" + this.TextBox1.Text+"',设备名称id=";
			string sql2="select * from sbname where 设备名称='" + this.DropDownList1.SelectedValue.ToString()+"'";

			db1.cleardata();
			db1.ExectueSQL("sbname",sql2);

			sql1+=db1.getdata().Tables["sbname"].Rows[0]["设备名称id"].ToString();
			sql1+=",厂家id=";
			...
			sql1+=db1.getdata().Tables["changjia"].Rows[0]["厂家id"].ToString();
			sql1+=",出厂日期='" + this.TextBox2.Text +"',实验室id=";
			...
			sql1+=db1.getdata().Tables["labname"].Rows[0]["实验室id"].ToString();
			sql1+=" where 设备id="+Request.QueryString["id"];
