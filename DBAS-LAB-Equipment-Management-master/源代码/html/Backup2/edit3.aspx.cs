...
namespace html
{
	...
	/// edit3 的摘要说明。
	/// </summary>
	public class edit3 : System.Web.UI.Page
	{
		...
		private void Button1_Click(object sender, System.EventArgs e)
		{
			...
			if(db1.getdata().Tables["admin"].Rows.Count==0)
			{
				...
				db1.ExectueSQL("userid",sql);
				if(db1.getdata().Tables["userid"].Rows.Count==0)
					Response.Redirect("cuowu4.aspx");
				else
				{
					sql="update userid set password='"+this.TextBox2.Text+"' where name='"+name+"'";
					db1.ExectueSQL("",sql);
				}
			}
			else
			{
				sql="update admin set password='"+this.TextBox2.Text+"' where username='"+name+"'";
				db1.ExectueSQL("",sql);
			}
